...
public class TracedSubscriber<T> implements SpanSubscription<T> {
     ...
     */
    public static <T> Function<? super Publisher<T>, ? extends Publisher<T>> asOperator(Supplier<Tracer> tracerSupplier) {
        return Operators.liftPublisher((publisher, sub) -> {
            ...
            return new TracedSubscriber<>(sub, sub.currentContext(), tracerSupplier.get());
        });
    }
